...
ssh -q -i id_rsa root@localhost -p 2222 -o "StrictHostKeyChecking no" 'mkdir /tmp/tpmea'
while test $? -gt 0
do
  sleep 5
  echo "[+] Waiting for vm to start..."
...
scp -rp -q -i id_rsa -P 2222 -o "StrictHostKeyChecking no" *.{go,mod,sum} root@localhost:/tmp/tpmea
...
echo "[+] Running tests ..."
ssh -i id_rsa root@localhost -p 2222 -o "StrictHostKeyChecking no" 'cd /tmp/tpmea && /usr/local/go/bin/go test -v -coverprofile=coverage.txt -covermode=atomic'

echo "[+] Copying code-coverage data..."
scp -rp -q -i id_rsa -P 2222 -o "StrictHostKeyChecking no" root@localhost:/tmp/tpmea/coverage.txt test/
